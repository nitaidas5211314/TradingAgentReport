...
把仓库里 TradingAgents 每次跑图产出的历史报告（美股与加密标的），变成一个可以浏览、
检索的网站，一键部署到 Vercel。
...
reports/<ticker>/<YYYY-MM-DD>/<HHMM-TZ>.json  # 结论、名称/行业等结构化字段
summary/<YYYY-MM-DD>.md                       # 每日决策汇总（可选，没有则自动隐藏该入口）
latest-summary.md                             # 最新一期汇总（可选）
```

解析逻辑集中在 [`lib/reports.ts`](lib/reports.ts)：

- 结论优先取 JSON 的 `decision`，缺失时回退到正文 `## 结论` 代码块、再回退到 `**Rating**`
- 名称 / 行业 / 交易所从 JSON 的 `state.instrument_context` 中提取
  （股票读 `Company:`，加密标的读 `Name:`）
- 交易日、分析师、耗时、模型等从正文开头的元信息行解析
- `**Entry Price**` / `**Stop Loss**` / `**Time Horizon**` / `**Executive Summary**` 提取到页头卡片
- 同一标的一天内多次跑图会按时间倒序全部保留
- `reports/<ticker>/latest.md` 是快捷副本，会被自动跳过，不会重复计数

股票与加密标的混合时，报告列表页会额外出现「资产类型」筛选项。
...
|---|---|
| `/` | 概览：统计、各标的最新结论、最近报告 |
| `/tickers` | 标的列表（名称、行业 / 类型、最新结论、报告数） |
| `/tickers/[ticker]` | 单个标的：最新结论卡片 + 全部历史报告 |
| `/reports` | 全部报告，支持按关键词 / 标的 / 类型 / 结论筛选，按交易日分组 |
| `/reports/[ticker]/[date]/[slug]` | 报告详情：结构化元信息 + 正文 + 目录导航 + 前后报告切换 |
| `/summary`、`/summary/[date]` | 每日决策汇总（仓库里存在 `summary/` 时才出现） |
...
报告是构建期读取的，所以**每次把新的 `reports/` 提交并 push 到仓库，Vercel 就会自动
重新构建并发布**。跑图脚本照常 commit 即可，不需要改动网站。
